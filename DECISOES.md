...
## [2026-08-15] D-317 � Workflow Issues+PRs no GitHub (padr�o oficial)

- Toda tarefa do PLANO_MESTRE vira Issue (label por fase, milestone "Open Beta Hardenada").
- Todo commit do Doer vai para branch `feature/<tarefa-id>` e vira PR com `Closes #<issue>`; main s� recebe merge via PR (CI verde + review do Thinker no GitHub).
- Cria��o de Issue/PR via `gh` CLI (GITHUB_TOKEN do CI) � sem depender do Operador. Documentado em `docs/WORKFLOW_GITHUB.md` + templates `.github/{PULL_REQUEST_TEMPLATE.md, ISSUE_TEMPLATE/tarefa.md}` + `CONTRIBUTING.md`.
- Exce��o: `chore:` de reconcilia��o de estado (PLANO_MESTRE/DECISOES) segue push direto em main.

---

## [2026-08-15] D-318 � Motion Principles obrigat�rios de UI (kylezantos/design-principles)

- Skeleton proporcional ao conte�do, lazy-load nativo (`loading=lazy` + blur placeholder), transi��es de rota (View Transitions API), bot�o async com loading+disabled, progresso n�o-bloqueante em todo conte�do ass�ncrono.
- Respeitar `prefers-reduced-motion` (reduced = fade 100ms; none = s� mudan�a de estado). Reutilizar Motion/GSAP/Anime.js (sem nova lib).
- Documentado em `docs/UI_MOTION_PRINCIPLES.md`; aplica��o faseada (p�blicas primeiro). Lighthouse > 90 � gate.

---

## [2026-08-15] D-319 � Stack observabilidade+qualidade expandida

- **Adotar agora (sem custo):** OpenTelemetry (traces/metrics/logs, backend-agnostic), Knip (c�digo/deps n�o usadas), Stryker (mutation testing), Arch-contract (regras de arquitetura), Comilint (lint de commits).
- **Mant�m:** Sentry (T293/T315), Playwright (T023), coverage v8 (T024).
- **Pend�ncia do Operador (custo):** Datadog vs NewRelic; Codecov (free open-source / ~US$10 privado); endpoint OTLP de produ��o.
- **Avaliar antes de substituir:** Biome vs ESLint+Prettier (benchmark real >2x + sem perder regras de seguran�a).
