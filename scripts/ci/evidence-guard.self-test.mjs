...
check(
  "aceita localhost com credenciais",
  validarDatabaseUrlLocal("postgresql://u:p@localhost:5434/db").ok,
);
...
check(
  "recusa railway.internal",
  !validarDatabaseUrlLocal("postgresql://postgres.railway.internal:5432/railway").ok,
);
check(
  "recusa railway.app",
  !validarDatabaseUrlLocal("postgresql://containers-us-west.railway.app:5432/r").ok,
);
check("recusa mediarate.app", !validarDatabaseUrlLocal("postgresql://db.mediarate.app:5432/r").ok);
check(
  "recusa host de produção por rótulo",
  !validarDatabaseUrlLocal("postgresql://meu-host.prod:5432/r").ok,
);
check("recusa host remoto qualquer", !validarDatabaseUrlLocal("postgresql://10.0.0.5:5432/r").ok);
...
check(
  "aceita localhost com db 'products'",
  validarDatabaseUrlLocal("postgresql://u:p@localhost:5432/products").ok,
);
...
check(
  "--spec=x",
  parseEvidenceArgs(["--spec=e2e/jornada-critica.spec.ts"]).spec === "e2e/jornada-critica.spec.ts",
);
...
check(
  "redige credenciais",
  !redigirUrl("postgresql://user:senha@localhost:5432/db").includes("senha"),
);
