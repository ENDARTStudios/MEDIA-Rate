      ...
      {PLANS.map((plan) => {
        ...
        return (
          ...
          >
            {isPlus && (
              <>
                ...
                <div className="absolute inset-0 rounded-md overflow-hidden pointer-events-none" aria-hidden="true">
                  <div className="absolute top-0 left-0 right-0 h-[1px]" style={{ background: "linear-gradient(90deg, transparent, #818CF8 50%, transparent)", animation: "beam-h 3s ease-in-out infinite" }} />
                  <div className="absolute bottom-0 left-0 right-0 h-[1px]" style={{ background: "linear-gradient(90deg, transparent, #818CF8 50%, transparent)", animation: "beam-h 3s ease-in-out infinite 1.5s" }} />
                  <div className="absolute left-0 top-0 bottom-0 w-[1px]" style={{ background: "linear-gradient(180deg, transparent, #818CF8 50%, transparent)", animation: "beam-v 3s ease-in-out infinite 0.5s" }} />
                  <div className="absolute right-0 top-0 bottom-0 w-[1px]" style={{ background: "linear-gradient(180deg, transparent, #818CF8 50%, transparent)", animation: "beam-v 3s ease-in-out infinite 2s" }} />
                </div>
            ...
            <div className="mb-6">
              <h3 className="text-lg font-heading font-bold text-[#EDE7DC] capitalize">{t(plan.id)}</h3>
            ...
            <ul className="space-y-3 mb-8 flex-1 text-sm">
              {(t.raw(`${plan.id}Features`) as string[]).map((f: string, i: number) => (
                <li key={i} className="flex items-start gap-2">
                  <svg className="w-4 h-4 mt-0.5 shrink-0 text-[#818CF8]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
