...
function searchMedia(query: string): SearchResult[] {
  ...
  return MOCK_MEDIA
    .filter((m: Media) => m.title.toLowerCase().includes(q))
    .slice(0, 15)
    .map((m: Media) => ({
      id: m.id,
      title: m.title,
      year: m.year,
      type: m.type === "movie" ? "Filme" : m.type === "series" ? "Série" : m.type === "game" ? "Game" : m.type === "anime" ? "Anime" : m.type,
...
export function SearchCommand() {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-modal flex items-start justify-center pt-[15vh]" role="dialog" aria-modal="true" aria-label="Busca global de mídia">
          <div className="fixed inset-0 bg-[#09090F]/80 backdrop-blur-sm" onClick={() => setOpen(false)} aria-hidden="true" />
          <div ref={containerRef} className="relative w-full max-w-lg bg-[#11111E] border border-[rgba(129,140,248,0.15)] rounded-lg shadow-floating overflow-hidden" onKeyDown={handleKeyDown}>
            <div className="flex items-center gap-3 px-4 py-3 border-b border-[rgba(129,140,248,0.08)]">
              <svg className="w-4 h-4 text-[#6B7280] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
              <input
                ref={inputRef}
              ...
              />
              {query && (
                <button onClick={() => { setQuery(""); inputRef.current?.focus(); }} className="text-[#6B7280] hover:text-[#9CA3AF]">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              )}
              <kbd className="hidden sm:inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-[#1C1C2E] text-[#6B7280] border border-[rgba(129,140,248,0.08)] font-mono">Esc</kbd>
            </div>
            ...
            <div className="max-h-[50vh] overflow-y-auto">
              ...
              {debouncedQuery.length >= 2 && totalResults === 0 && (
                <div className="px-4 py-10 text-center">
                  ...
                  <p className="text-xs text-[#6B7280]">Tente outro título ou explore o catálogo.</p>
                </div>
              )}

              {debouncedQuery.length >= 2 && Object.entries(grouped).map(([group, items]) => (
                <div key={group}>
                  <div className="px-4 py-2 text-[10px] text-[#6B7280] uppercase tracking-widest font-medium bg-[#0D0D1A] border-b border-[rgba(129,140,248,0.04)]">{group}</div>
                  {items.map((item) => {
                    const idx = flatIdx++;
                    ...
                    return (
                      <button
                        key={item.id}
                        onClick={() => { router.push(`/media/${item.id}`); setOpen(false); }}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-[#EDE7DC] font-heading truncate">{item.title}</p>
                          <p className="text-xs text-[#6B7280]">{item.type} &middot; {item.year}</p>
                        </div>
                        {item.score != null && (
                          <span className="text-xs font-mono font-bold shrink-0 px-1.5 py-0.5 rounded" style={{ color: scoreColor(item.score, "0-100"), backgroundColor: `${scoreColor(item.score, "0-100")}15` }}>
                            {item.score}
                          </span>
                        )}
                      </button>
