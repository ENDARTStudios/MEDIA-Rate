...
export function MediaScoreModule({ score, mediaType }: MediaScoreModuleProps) {
  ...
  const confidenceLabels: Record<string, string> = { high: t("highConfidence"), medium: t("mediumConfidence"), low: t("lowConfidence") };

  return (
    <div className="bg-[#11111E] rounded-2xl p-6 border border-[#1C1C2E] space-y-5" data-testid="score-module">
      <div className="flex items-center gap-5">
        <div className="relative w-[120px] h-[120px] shrink-0">
          <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120" role="img" aria-label={t("scoreAriaLabel", { score: consolidated, sources: sources.length })}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            {confidence === "low" && (
              <span className="inline-flex items-center text-[#F59E0B] cursor-help" title={t("fewSources")}>
          ...
          {updatedAt && (
            <p className="text-xs text-[#6B7280] mb-1" data-testid="last-updated">
              ...
              {isStale && <span className="ml-1 px-1.5 py-0.5 bg-[#F59E0B]/15 text-[#F59E0B] rounded text-[10px]">{t("mayBeOutdated")}</span>}
            </p>
          )}
      ...
      <div className="space-y-2 pt-3 border-t border-[#1C1C2E]">
        <p className="text-xs text-[#6B7280] font-medium uppercase tracking-wider">{t("sources")}</p>
        {sources.map((s) => {
