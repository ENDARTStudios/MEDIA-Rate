  ...
  for (const locale of LOCALES) {
    test(locale + " — 0 raw ID, 0 raw key, real titles, counts correct", async () => {
      ...
      try {
        ...
        await page.goto(PROD + "/" + locale + "/login", {
        ...
        const suspiciousLines = lines.filter((l) =>
          /^\s*(?:Mídia|[0-9a-f]{8,})\s*$/.test(l.trim()),
        );
        ...
        const expectedMoveTo =
          locale === "pt-BR" ? "Mover para" : locale === "en-US" ? "Move to" : "Mover a";
        ...
        const hasContent =
          bodyText.includes("Quero Ver") ||
          bodyText.includes("Want to") ||
          bodyText.includes("Quiero ver");
