...
async function main() {
  ...
  const apiResult = await page.evaluate(async ({ name, email, pass, origin }: { name: string; email: string; pass: string; origin: string }) => {
    try {
      const resp = await fetch(origin + "/api/v1/auth/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, email, password: pass, confirmPassword: pass, acceptTerms: true }),
        credentials: "include",
      });
  ...
  if (!ok) {
    ...
    const loginResult = await page.evaluate(async ({ email, pass, origin }: { email: string; pass: string; origin: string }) => {
      try {
  ...
  for (const [url, label] of [
    ...
    await page.screenshot({ path: path.resolve(__dirname, "..", "e2e", "screenshots", `t174-${label}.png`), fullPage: false });
    console.log(`  ✓ ${label}`);
  }
