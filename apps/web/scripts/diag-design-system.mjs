...
// Validate
let pass = true;
if (scoreDials === 0) { console.log("[FAIL] 0 score dials rendered"); pass = false; }
if (mediaCards === 0) { console.log("[FAIL] 0 media cards rendered"); pass = false; }
if (buttons === 0) { console.log("[FAIL] 0 buttons rendered"); pass = false; }
if (perr.length > 0) { console.log("[FAIL] pageerrors detected:", perr); pass = false; }

const isBgDark = bgColor.includes("9, 9, 15") || bgColor === "rgb(9,9,15)";
if (!isBgDark) { console.log("[WARN] bgColor is not #09090F:", bgColor); }
else { console.log("[PASS] bgColor #09090F confirmed"); }
