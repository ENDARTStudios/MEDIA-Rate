...
const CURADORIA: CuradoriaItem[] = [
  {
    origemFonte: "tmdb",
    origemFonteId: "438631", // Dune (2021)
    destinoFonte: "openlibrary",
    destinoFonteId: "duna", // livro Duna
    tipo: "ADAPTACAO_DE",
    notaEditorial: "Baseado no livro de Frank Herbert",
  },
  {
    origemFonte: "tmdb",
    origemFonteId: "693134", // Dune: Part Two (2024)
    destinoFonte: "openlibrary",
    destinoFonteId: "duna", // livro Duna
    tipo: "ADAPTACAO_DE",
    notaEditorial: "Baseado no livro de Frank Herbert",
  ...
  {
    origemFonte: "tmdb",
    origemFonteId: "13183", // Watchmen (2009)
    destinoFonte: "comicvine",
    destinoFonteId: "watchmen", // HQ Watchmen
    tipo: "ADAPTACAO_DE",
    notaEditorial: "Adaptação da graphic novel de Alan Moore",
  },
  {
    origemFonte: "jikan",
    origemFonteId: "berserk", // mangá Berserk (ANIME)
    destinoFonte: "tmdb_tv",
    destinoFonteId: "2509", // série Berserk (1997)
    tipo: "ADAPTACAO_DE",
    notaEditorial: "Adaptação do mangá de Kentaro Miura",
  },
  {
    origemFonte: "tmdb_tv",
    origemFonteId: "82856", // Better Call Saul
    destinoFonte: "tmdb_tv",
    destinoFonteId: "1396", // Breaking Bad
    tipo: "SPINOFF_DE",
    notaEditorial: "Spin-off de Breaking Bad",
  },
  {
    origemFonte: "tmdb",
    origemFonteId: "603", // The Matrix (1999)
    destinoFonte: "tmdb",
    destinoFonteId: "604", // The Matrix Reloaded
    tipo: "SEQUENCIA_DE",
    notaEditorial: "Sequência de The Matrix",
  },
  {
    origemFonte: "tmdb",
    origemFonteId: "604", // The Matrix Reloaded
    destinoFonte: "tmdb",
    destinoFonteId: "605", // The Matrix Revolutions
    tipo: "SEQUENCIA_DE",
    notaEditorial: "Sequência de Reloaded",
  },
  {
    origemFonte: "tmdb",
    origemFonteId: "438631", // Dune (2021)
    destinoFonte: "tmdb",
    destinoFonteId: "693134", // Dune: Part Two
    tipo: "SEQUENCIA_DE",
    notaEditorial: "Sequência de Dune",
  },
  // Pares sem ids confiáveis — resolvem por título normalizado (fallback).
  {
    origemTitulo: "The Matrix",
    destinoTitulo: "The Matrix",
    tipo: "MESMO_UNIVERSO",
    notaEditorial: "Franquia multiplataforma",
  },
  {
    origemTitulo: "A Odisseia",
    destinoTitulo: "A Odisseia",
    tipo: "ADAPTACAO_DE",
    notaEditorial: "Baseado no poema épico de Homero",
  },
  {
    origemTitulo: "Os Irmãos Karamazov",
    destinoTitulo: "Os Irmãos Karamazov",
    tipo: "ADAPTACAO_DE",
    notaEditorial: "Baseado no romance de Dostoiévski",
  },
  {
    origemTitulo: "The Boys",
    destinoTitulo: "The Boys",
    tipo: "ADAPTACAO_DE",
    notaEditorial: "Adaptação da HQ de Garth Ennis",
  },
  {
    origemTitulo: "The Lord of the Rings",
    destinoTitulo: "The Lord of the Rings",
    tipo: "ADAPTACAO_DE",
    notaEditorial: "Baseado na obra de J.R.R. Tolkien",
  },
];
...
async function classificarGeneros(prisma: PrismaClient): Promise<void> {
  const generos = await prisma.genero.findMany({
    select: { id: true, slug: true, tipo: true, midia_alvo: true },
  });
  let atualizados = 0;
  for (const g of generos) {
    if (NARRATIVOS.includes(g.slug)) {
      if (g.tipo !== "NARRATIVO") {
        await prisma.genero.update({
          where: { id: g.id },
          data: { tipo: "NARRATIVO", midia_alvo: null },
        });
        atualizados++;
      }
      continue;
    ...
    if (alvo) {
      if (g.tipo !== "SUBGENERO" || g.midia_alvo !== alvo) {
        await prisma.genero.update({
          where: { id: g.id },
          data: { tipo: "SUBGENERO", midia_alvo: alvo },
        });
        atualizados++;
      }
    }
  ...
  console.log(
    `[seed:relacoes] ${atualizados} gêneros classificados (${NARRATIVOS.length} narrativos / ${Object.keys(SUBGENEROS).length} subgêneros).`,
  );
}
