...
describe("UploadService (unit)", () => {
  ...
  it("validate — GIF valido (magic bytes 47 49 46 38)", () => {
    ...
    const result = service.validateAndPrepare({ buffer: buf, mimetype: "image/gif", originalname: "test.gif" });
  ...
  it("validate — WebP valido (magic bytes 52 49 46 46)", () => {
    ...
    const result = service.validateAndPrepare({ buffer: buf, mimetype: "image/webp", originalname: "test.webp" });
  ...
  it("validate — PDF valido (magic bytes 25 50 44 46)", () => {
    ...
    const result = service.validateAndPrepare({ buffer: buf, mimetype: "application/pdf", originalname: "doc.pdf" });
  ...
  it("validate — buffer vazio (0 bytes) lanca BadRequestException nos magic bytes", () => {
    ...
    expect(() => service.validateAndPrepare({ buffer: buf, mimetype: "image/jpeg", originalname: "empty.jpg" }))
      .toThrow(BadRequestException);
  });
  ...
  it("validate — filename com multiplos pontos usa ultima extensao", () => {
    const buf = Buffer.alloc(16, 0);
    buf[0] = 0xFF; buf[1] = 0xD8; buf[2] = 0xFF;
    const result = service.validateAndPrepare({ buffer: buf, mimetype: "image/jpeg", originalname: "photo.test.final.jpg" });
    expect(result.filename).toMatch(/\.jpg$/);
  });
  ...
  it("validate — SHA-256 hex gerado corretamente", () => {
    ...
    buf[0] = 0xFF; buf[1] = 0xD8; buf[2] = 0xFF;
    const result = service.validateAndPrepare({ buffer: buf, mimetype: "image/jpeg", originalname: "hash.jpg" });
    expect(result.sha256).toHaveLength(64);
  ...
  it("validate — GIF magic bytes errados (apenas primeiros 2 corretos) lanca erro", () => {
    const buf = Buffer.alloc(16, 0);
    buf[0] = 0x47; buf[1] = 0x49; buf[2] = 0x00; buf[3] = 0x00;
    expect(() => service.validateAndPrepare({ buffer: buf, mimetype: "image/gif", originalname: "bad.gif" }))
      .toThrow(BadRequestException);
  });
});
