...
describe("Zod Validation (T1.4)", () => {
  ...
  beforeAll(async () => {
    ...
    // T230: AppModule completo � timeout expl�cito (contenda de 80 arquivos
    // paralelos pode estourar o default de 5s).
  }, 60_000);

  afterAll(async () => {
    // T230: defensivo � se o beforeAll falhou, app pode n�o existir; e
    // falha de teardown n�o deve derrubar o arquivo.
    if (!app) return;
    try {
      await app.close();
    } catch {
  ...
  it("POST /api/v1/echo com body valido → 201 + echo", async () => {
