...
function mockPrisma({ itens, total, porStatus }: { itens: any[]; total: number; porStatus: any[] }) {
  return {
...
describe("InteracoesService.listar (D-525)", () => {
  ...
  it("cursor opaco avança a página; página curta encerra a paginação", async () => {
    ...
    const r2 = await pagina2.listar("u1", { limit: 2, cursor: r.nextCursor! });
    ...
    const r3 = await ultima.listar("u1", { limit: 2, cursor: r2.nextCursor! });
