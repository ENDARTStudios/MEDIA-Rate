...
export class UploadService {
  ...
  validateAndPrepare(file: { buffer: Buffer; mimetype: string; originalname: string }) {
    ...
    this.logger.log(
      `Upload validado: ${filename} (${file.buffer.length} bytes, SHA-256: ${sha256.slice(0, 8)}...)`,
    );
    return {
      filename,
      buffer: file.buffer,
    ...
    for (let i = 0; i < magic.length; i++) {
      if (buffer[i] !== magic[i]) {
        throw new BadRequestException(
          `Tipo MIME declara ${mimetype} mas os magic bytes não conferem. Rejeitado por segurança.`,
        );
      }
    }
  }
