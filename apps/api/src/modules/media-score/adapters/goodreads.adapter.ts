...
/**
 * Goodreads — sem API pública estável; página do livro expõe JSON-LD
 * aggregateRating.ratingValue (0–5, público). Scraping numérico ISOLADO —
 * gates: MEDIA_PREPARACAO_ENABLED=true (fase preparação, §IV).
 */
export class GoodreadsAdapter implements FonteAdapter {
  ...
  async coletar(consulta: ConsultaMedia): Promise<NotaColetada[]> {
    const slug =
      consulta.idsExternos?.goodreads ??
      consulta.titulo
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "");
    const url = `https://www.goodreads.com/book/show/${slug}`;
