...
| T043 ZAP policy | `PENDENTE` | `requires_tdd: false`; `espec` não recebida pelo usuário; `P011` vazio (`URL` real via `vercel[bot]`); `P012` (`settings`) `NÃO` `VERIFICÁVEL`; `P013` (`GHAS` `exclusivo` `ou` `exceção`) `PENDENTE` — `D-483`: `FAIL-NEW: 0`, `WARN-NEW: 15` (`info-disclosure` `serious` `CSP` `COEP` `Base64` `permissions-policy` `Cross-Domain` `auth` `Sec-Fetch-Dest`) `+` `PASS: 55`; `fail_action: true` `FAIL`; `creation` `Resource` `not` `accessible` (`token` `P012` `restritivo`) |
