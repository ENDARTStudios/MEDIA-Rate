...
- [x] 4.15 Contrato público de `GET /api/v1/interacoes` com DTO ALLOWLIST (T036/B3, D-536): `interacoes-response.dto.ts` + `interacoes.mapper.ts` removem colunas internas/legadas (`usuario_id`, `tenant_id`, `created_at`, `tipo`, `rating`, `comentario` plaintext) e preservam `reacao`/`motivo_abandono`/`midia` (store/feed). Envelope `{items,total,porStatus,nextCursor}` e Swagger mantidos. TDD: `interacoes-dto.spec.ts` + reforço em `interacoes-lista.spec.ts`. Sem migrations; PR sem merge.
