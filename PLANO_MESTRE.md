...
- [~] 9.16 Triagem de workflows crônicos na `main` (T046/D-541): `Release` (raiz do monorepo sem script `build` → `npm error Missing script: "build"`) e `create-pr-from-branch` (heredoc em coluna 0 → **YAML inválido** → run sem jobs em todo push). Ambos **não-required**. Correlatos sem correção: `create-pr-from-branch` só rodará em `feature/**` após o fix; `Release` vira manual. Correção em PR sem merge; ver `docs/CI.md`.
