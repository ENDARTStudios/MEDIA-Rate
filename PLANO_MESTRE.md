...
- [x] 8.7 Testes de carga (k6) simulando 1.000 usuários. · evid: T220 - k6-scripts/load-test.js com ramp-up 0→1000 VUs em 5min + sustain 10min + ramp-down 5min (stages, executor ramping-vus), 3 cenários mantidos (health 600 VUs / catálogo 250 / checkout 150 por faixa de __VU); config.js (BASE_URL, VUS_MAX=1000, STAGES); thresholds http_req_duration p95<500ms, http_req_failed<1%, http_reqs>10000, compat T8.4; docs/LOAD_TESTING.md completo (execução, interpretação, quando executar, instalação); smoke test validado: exit 0 (10 VUs/30s, p95≈4ms, erro 0%, ~110k reqs); detecção de smoke em runtime (CLI override troca cenário load→default; execs sem sleep de leitura no smoke).
...
Após este plano ser commitado, o Doer procura o primeiro `[ ]` de cima para baixo: **Fase 0, tarefa 0.1**. Já está feita no MVP? Re-verificar com evidência. Se passar, marcar `[x]` e seguir. Se não, executar- [x] 4.7 Módulo dmin. · evid: T221 - GET /api/v1/admin/stats com métricas REAIS (AdminService: usuarios total/ativos_7d, midias total/por_tipo excluindo soft delete, watchlists entries/usuarios, sessoes ativas, planos free/plus/premium), cache 60s (chave admin:stats, X-Cache HIT/MISS), @Roles('ADMIN') (403 non-admin), audit ADMIN_STATS_VIEWED, resposta tipada sem PII, banco vazio -> zeros. AdminModule importa AuthModule (guards) e prove AuditLogService.
