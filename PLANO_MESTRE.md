...
- [~] 2.10 Criptografia de coluna (email/telefone): `ColumnEncryptionService` (AES-256-GCM) criado, **não wired** nas colunas. **T048/D-542:** análise de viabilidade **docs-only** — **não** implementar agora (e-mail buscável por igualdade × IV não determinístico; exige migration+backfill e secret `COLUMN_ENCRYPTION_KEY`) → decisão em **P017**; follow-up de baixo risco: mascarar PII em log (`auth.service.ts:261`). Ver `docs/LGPD_DADOS.md`. **T049/D-543:** mascaramento de PII em logs de `auth` **implementado** (helper `pii-mask.ts` + `auth-pii-log.spec.ts`); cifragem de coluna **segue bloqueada** (P017).
